...
export default function Home({ treinos }) {
  const [indexTreino, setindexTreino] = useState(treinos.length - 1)
  ...
  return (
    <>
      ...
      {treinos[indexTreino].Exercicios.map((exercicio_elem) => (
        <TreinoComponent key={exercicio_elem._id} exercicio={exercicio_elem} quantidadeExercicios={treinos[indexTreino].length}></TreinoComponent>
      ))}
